...
$Selenium->RunTest(
    sub {
        ...
        # Create test article.
        my $ArticleID = $Kernel::OM->Get('Kernel::System::Ticket::Article')->BackendForChannel( ChannelName => 'Phone' )
            ->ArticleCreate(
            TicketID             => $TicketID,
            SenderType           => 'customer',
            From                 => $CustomerUsers[1]->{UserMailString} . ', ' . $CustomerUsers[4]->{UserMailString},
            To                   => 'Customer B <customer-b@example.com>',
        ...
        $Selenium->WaitFor(
            JavaScript =>
                "return typeof(\$) === 'function' && \$('#TicketCustomerContentToCustomer .CustomerQueue').length == 2;"
        );

        # Verify that the first and the last customer users are pre-selected for 'To' field.
        for my $CustomerUser (@CustomerUsers) {
            my $Length = $CustomerUser->{IsFound} ? 1 : 0;
            my $IsFound = $Length ? 'is found' : 'is not found';

            $Self->Is(
                $Selenium->execute_script(
                    "return \$('#TicketCustomerContentToCustomer .CustomerQueue[value*=\"$CustomerUser->{UserFirstname}\"]').length;"
                ),
                $Length,
                "CustomerUser login '$CustomerUser->{UserLogin}' $IsFound"
            );
        }
