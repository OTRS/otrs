...
$Selenium->RunTest(
    sub {
        ...
        for my $GroupUserRelation (@GroupUserRelations) {
            $Success = $GroupObject->GroupMemberAdd(
        ...
        # check data
        my @ExpectedMessages = (
            $Tickets[0]->{TicketNumber} . ': Ticket locked.',
        ...
        $Selenium->execute_script("\$('#PriorityID').val('4').trigger('change');");
        $Selenium->execute_script("\$('#StateID').val('2').trigger('change');");
        ...
        for my $ConfigValue ( 0 .. 1 ) {
            ...
            for my $Test (@Tests) {
                ...
                $Selenium->execute_script(
                    "\$('#QueueID').val('$Test->{SelectedQueueID}').trigger('change');"
                );
