...
$Selenium->RunTest(
    sub {
        ...
        for my $Type (qw(Ticket Article)) {
            ...
            $Selenium->find_element( "#Name", 'css' )->send_keys("");
            ...
            $Selenium->WaitFor( JavaScript => 'return !$("#Key_3:visible").length;' );
            ...
            $Self->IsDeeply(
                ...
                [ 'Key1', 'Key2' ],
                "[ 'Key1', 'Key2' ] is possible #DefaultValue",
            );

            $Selenium->execute_script(
                "\$('button[type=submit]')[0].scrollIntoView(true);",
            );
            ...
            # Check for saved 'default values' on creation, expecting both values to be present.
            $Self->IsDeeply(
                $Selenium->execute_script(
                    "return \$('#DefaultValue').val();"
                ),
            ...
            $Selenium->WaitFor( JavaScript => 'return $("#PossibleNone").val() == "1";' );

            $Selenium->execute_script("\$('#TreeView').val('1').trigger('redraw.InputField').trigger('change');");
            $Selenium->WaitFor( JavaScript => 'return $("#TreeView").val() == "1";' );

            $Selenium->execute_script("\$('#ValidID').val('2').trigger('redraw.InputField').trigger('change');");
            $Selenium->WaitFor( JavaScript => 'return $("#ValidID").val() == "2";' );
