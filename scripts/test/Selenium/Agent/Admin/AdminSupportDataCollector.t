...
my %Setting = $Kernel::OM->Get('Kernel::System::SysConfig')->ConfigItemGet(
    Name => 'Frontend::Module###AdminSupportDataCollector',
);

if ( defined $Setting{Valid} && !$Setting{Valid} ) {
    $Self->False(
        $Setting{Valid},
        'AdminSupportDataCollector is disabled, skip the test'
    );
...
$Selenium->RunTest(
    sub {
        ...
        $Selenium->execute_script(
            "\$('.ContentColumn div.WidgetSimple a').click();"
        );
        ...
        %Setting = $Kernel::OM->Get('Kernel::System::SysConfig')->ConfigItemGet(
        ...
        if ( defined $Setting{Valid} && !$Setting{Valid} ) {
            $Self->False(
                $Setting{Valid},
                'AdminRegistration is disabled, skip the test'
            );
