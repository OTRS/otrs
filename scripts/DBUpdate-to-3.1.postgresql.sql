-- ----------------------------------------------------------
--  driver: postgresql, generated: 2011-09-13 12:24:14
-- ----------------------------------------------------------
...
CREATE TABLE gi_object_lock_state (
    ...
    create_time timestamp(0) NOT NULL,
    change_time timestamp(0) NOT NULL,
    CONSTRAINT gi_object_lock_state_U_811 UNIQUE (webservice_id, object_type, object_id)
);
...
CREATE TABLE dynamic_field (
    ...
    change_by INTEGER NOT NULL,
    PRIMARY KEY(id),
    CONSTRAINT dynamic_field_U_222 UNIQUE (name)
);
SET standard_conforming_strings TO ON;
