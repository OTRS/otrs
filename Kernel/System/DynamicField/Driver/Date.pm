...
sub new {
    ...
    my $DynamicFieldDriverExtensions = $Self->{ConfigObject}->Get('DynamicFields::Extension::Driver::Date');
    ...
    if ( $Value && IsHashRefWithData($Value) ) {
            ...
            )
        {
            ...
            if ( $Start eq 'Before' ) {
                ...
                my ( $NSec, $NMin, $NHour, $NDay, $NMonth, $NYear, $NWeekDay ) = $Self->{TimeObject}->SystemTime2Date(
                    SystemTime => $Now,
                );
