...
# --
# $Id: Backend.pm,v 1.62 2012-01-02 20:22:10 cr Exp $
# --
...
get a dynamic field value tyoe.

    my $ValueType = $BackendObject->ValueTypeGet(
        DynamicFieldConfig => $DynamicFieldConfig,      # complete config of the DynamicField
    );

    Return $ValueType                                   # depends on backend type, Possible resutls:
...
sub ValueTypeGet {
    ...
    # call ValueTypeGet on the specific backend
    return $Self->{$DynamicFieldBackend}->ValueTypeGet(%Param);
}
...
$Revision: 1.62 $ $Date: 2012-01-02 20:22:10 $
