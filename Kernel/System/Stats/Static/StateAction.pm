...
sub Run {
    ...
    for my $StateID ( sort { $States{$a} cmp $States{$b} } keys %States ) {
        $States{$StateID} = $LanguageObject->Get( $States{$StateID} );
    ...
    while ( $TimePiece->mon() == $Param{Month} ) {
        ...
        my $Text = $LanguageObject->Get( $TimePiece->wdayname() ) . ' ' . $TimePiece->mday();
    ...
    my $Title = "$Year-$Month";
    return ( [$Title], [ $LanguageObject->Get('Days'), @Days ], @Data );
}
