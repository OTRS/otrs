#!/usr/bin/perl
# --
# otrs.WebServerInstaller.pl - helps to install OTRS PSGI Webserver on Microsoft Windows OS
...
if ( !Win32::IsAdminUser() ) {
    print "To be able to install or remove the Web Server, call the script with UAC enabled.\n";
...
my %ServiceConfig = (
    name        => "OTRSWebServer",
    display     => "OTRS Web Server",
    description => 'The OTRS Web Server service for Windows',
    path        => $^X,
    user        => '',
    passwd      => '',
    parameters  => '"' . $WorkDir . '\otrs.WebServer4win.pl' . '" -a servicestart',
);
